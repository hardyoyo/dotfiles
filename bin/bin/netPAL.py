...
# ruff: noqa: E501 # ignore line length
...
def net_info(target, quiet, count, latency_threshold):
    """
    A script to gather network-related information including active interface, 
    MTU, ping latency, and network speeds. 
    """
    if not quiet:
        click.echo("Gathering network information...")
    try:
        # Ping the target to determine the active interface
        ping_cmd = f"ping -c {count} {target}" if platform.system() == 'Darwin' else f"ping -c {count} {target} -W 1"
        ...
        if platform.system() == 'Darwin':
            ...
            route_cmd = f"ip route get {target} | grep -Po '(?<=dev )(\S+)'"
            route_result = subprocess.run(route_cmd, shell=True, stdout=subprocess.PIPE, text=True, check=True) 
            active_interface = route_result.stdout.strip()
        ...
        mtu_cmd = f"networksetup -getMTU {active_interface}" if platform.system() == 'Darwin' else f"ip link show {active_interface} | grep -Po '(?<=mtu )(\d+)'"
        ...
        if not quiet:
            ...
            valid_mtu_range_cmd = f"networksetup -listvalidMTUrange {active_interface}" if platform.system() == 'Darwin' else f"ip link show {active_interface} | grep -Po '(?<=mtu )(\d+)-(\d+)'"
    ...
    """
    A script to gather network-related information including active interface, 
    MTU, ping latency, and network speeds. Useful for answering the question:
    what network interface am I actually using, and is it any good?
    """
    if not quiet:
        click.echo("Gathering network information...")
    try:
