...
# make the shift_lock key act like left alt
Map ${SHIFT_LOCK} ${L_ALT}
...
# make the pause key act like mute
Map ${PAUSE} ${VOLMUTE}

# make the printsreen key act like volume up
Map ${PRINTSCREEN} ${VOLUP}

# make the scroll lock key act like volume down
Map ${SCROLLLOCK} ${VOLDOWN}

# OK, fine, maybe we need some kind of capslock, use numlock
Map ${NUMLOCK} ${SHIFT_LOCK}
